...
Experiments (Kimi round 3: four graphs, zero gradient steps):
  1. best_of_n   success vs N candidates (brain ranks all N in one call), self-check off to isolate ranking
  2. verifier    success with vs without the self-check (retry on detected failure)
  3. memory      first-try success over successive episodes, with vs without episodic memory
  4. speed       decision latency vs N for each brain profile, and the largest N that fits a time budget
...
def best_of_n(seeds, ns=(1, 2, 4, 8, 16)):
    ...
    for n in ns:
        res = [loop.attempt(f"pick up the {o}", o, brain.StubBrain("instant"), None, n=n, seed=s, verify=False)
               for o in OBJECTS for s in range(seeds)]
...
def verifier(seeds, n=4):
    ...
    for v in (False, True):
        res = [loop.attempt(f"pick up the {o}", o, brain.StubBrain("instant", verdict_error=0.05, seed=s), None,
                            n=n, seed=s, verify=v) for o in OBJECTS for s in range(seeds)]
...
def to_md(r):
    ...
    L += ["", "## 3. Memory (first-try success, first half vs second half of the run)", ""]
    for k, c in r["memory"].items():
        h = len(c) // 2
        L.append(f"- {k}: {sum(e['first_try'] for e in c[:h])}/{h} -> {sum(e['first_try'] for e in c[h:])}/{len(c) - h}")
    L += ["", f"## 4. Brain speed (decision = choose + rank all N, budget {r['speed'][0]['budget_s']} s)", "",
          "| brain | " + " | ".join(f"N={n}" for n in r["speed"][0]["decision_s"]) + " | max N in budget |",
          "|---|" + "---|" * (len(r["speed"][0]["decision_s"]) + 1)]
    L += [f"| {x['profile']} | " + " | ".join(f"{v:.2f}s" for v in x["decision_s"].values()) +
...
def main():
    ...
    r = {"when": time.strftime("%Y-%m-%d %H:%M"), "seeds": seeds, "best_of_n": best_of_n(seeds),
         "verifier": verifier(seeds), "memory": memory_curve(16 if a.quick else 40), "speed": speed(a.budget)}
